...
public class BatchMaster extends AppCompatActivity {
    VideoBatchMasterAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_batch_master);
        getWindow().setFlags(WindowManager.LayoutParams.FLAG_SECURE,
                WindowManager.LayoutParams.FLAG_SECURE);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        context = BatchMaster.this;
...
//        progressDialog.getWindow().setGravity(Gravity.BOTTOM | Gravity.CENTER);
    ...
    private void getMenuList() {
        ...
        try {
            ...
            Call<List<VideoBatchModel>> call = RetrofitClient.getInstance().getMyApi().getBatchMaster(mobile);
            call.enqueue(new Callback<List<VideoBatchModel>>() {
                @Override
                public void onResponse(Call<List<VideoBatchModel>> call, Response<List<VideoBatchModel>> response) {

                    if (progressDialog.isShowing())
                        progressDialog.dismiss();
                    //    Toast.makeText(context, "Size is " + response.body().size(), Toast.LENGTH_SHORT).show();
                    if (response.body() != null) {
                        List<VideoBatchModel> saravMenuModels = response.body();
                        try {
                            adapter = new VideoBatchMasterAdapter((ArrayList) saravMenuModels, context);
                ...
                @Override
                public void onFailure(Call<List<VideoBatchModel>> call, Throwable t) {
                    if (progressDialog.isShowing())
                        progressDialog.dismiss();
