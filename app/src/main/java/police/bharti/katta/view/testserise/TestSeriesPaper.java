...
public class TestSeriesPaper extends AppCompatActivity {
    TestSeriesPaperAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_test_series_paper);
 /*       getWindow().setFlags(WindowManager.LayoutParams.FLAG_SECURE,
                WindowManager.LayoutParams.FLAG_SECURE);*/
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        context = TestSeriesPaper.this;
...
//        progressDialog.getWindow().setGravity(Gravity.BOTTOM | Gravity.CENTER);
    ...
    private void getMenuList() {
        ...
        try {
            ...
            Call<List<TestPaperModel>> call = RetrofitClient.getInstance().getMyApi().getTestPaper(Preferences.get(context,Preferences.USER_MOBILE).toString(),Preferences.get(context,Preferences.SELECTEDTESTMENU),"1");
            call.enqueue(new Callback<List<TestPaperModel>>() {
                @Override
                public void onResponse(Call<List<TestPaperModel>> call, Response<List<TestPaperModel>> response) {

                    if (progressDialog.isShowing())
                        progressDialog.dismiss();

                    if (response.body() != null) {
                        List<TestPaperModel> saravMenuModels = response.body();
                        try {
                            adapter = new TestSeriesPaperAdapter((ArrayList) saravMenuModels, context);
                ...
                @Override
                public void onFailure(Call<List<TestPaperModel>> call, Throwable t) {
                    if (progressDialog.isShowing())
                        progressDialog.dismiss();
